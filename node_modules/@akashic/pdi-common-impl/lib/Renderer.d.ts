 ...
 */
export declare abstract class Renderer implements pdi.Renderer {
    begin(): void;
    abstract clear(): void;
    /**
     * 指定されたSurfaceの描画を行う。
     ...
     */
    abstract drawImage(surface: pdi.Surface, offsetX: number, offsetY: number, width: number, height: number, destOffsetX: number, destOffsetY: number): void;
    abstract drawSprites(surface: pdi.Surface, offsetX: number[], offsetY: number[], width: number[], height: number[], canvasOffsetX: number[], canvasOffsetY: number[], count: number): void;
     ...
     * 本メソッドの呼び出しは `Renderer#end()` から `Renderer#begin()` の間でなければならない。
     */
    abstract _putImageData(imageData: pdi.ImageData, dx: number, dy: number, dirtyX?: number, dirtyY?: number, dirtyWidth?: number, dirtyHeight?: number): void;
    end(): void;
}
