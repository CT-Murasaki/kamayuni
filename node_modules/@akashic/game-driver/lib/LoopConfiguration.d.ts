...
interface LoopConfiguration {
     ...
     * 目標のフレームからの遅れと判断する閾値。
     *
     * 初期値は6(`GameLoop.DEFAULT_DELAY_IGNORE_THERSHOLD`)。
     ...
     * 早送り倍率。
     *
     * 初期値は100(`GameLoop.DEFAULT_SKIP_TICKS_AT_ONCE`)。正の整数でなければならない。
     ...
     * `Game` 側で早送りとして取り扱う閾値。
     *
     * 初期値は30000(`GameLoop.DEFAULT_SKIP_THRESHOLD`)。
     ...
     * 初期値は90000(仮; GameLoop.DEFAULT_JUMP_TRY_THRESHOLD)。
     ...
     * 取得したスナップショットを無視する閾値。
     *
     * 初期値は15000(仮; GameLoop.DEFAULT_JUMP_IGNORE_THRESHOLD)。
    ...
    /**
     * 最新ティックの取得を要求する間隔。
     * 初期値は10000(仮; GameLoop.DEFAULT_TICK_POLLING_THRESHOLD)
     */
     ...
     * `loopMode` が `Replay` でない場合、この値は無視される。
     *
     * 他の箇所の注釈なしの「時刻」との違いに注意すること。この値は日付を含む絶対的な時刻情報 (ex. `Date.now()`) を扱う。
     */
    originDate?: number;
}
