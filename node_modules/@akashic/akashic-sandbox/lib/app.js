...
module.exports = function (options) {
    ...
    app.use("/game/*.js$", function (req, _res, next) {
        req.baseDir = app.gameBase;
        next();
    });
    app.use("/raw_game/*.js$", function (req, _res, next) {
        req.baseDir = app.gameBase;
        req.useRawScript = true;
        next();
    });
    app.use("/cascade/:index/*.js$", function (req, _res, next) {
        req.baseDir = app.cascadeBases[Number(req.params.index)];
        next();
    });
    app.use("/raw_cascade/:index/*.js$", function (req, _res, next) {
        req.baseDir = app.cascadeBases[Number(req.params.index)];
        req.useRawScript = true;
        next();
    });
    ...
    app.use("^\/game$", function (_req, res, _next) {
        res.redirect("/game/");
    });
    // /js/ /css/ /thirdparty/ を静的ファイルとして参照できるようにする
    app.use("/js/:version/engineFilesV*.js", function (req, res, next) {
        var libName = "ae".concat(req.params.version);
        var engineFilesName = req.originalUrl.replace("/js/".concat(req.params.version, "/"), "");
        var engineFilesPath = path.join(path.dirname(require.resolve(libName)), "dist/raw/debug/full/".concat(engineFilesName));
        if (fs.existsSync(engineFilesPath)) {
            var engineFilesSrc = fs.readFileSync(engineFilesPath).toString();
    ...
    app.use("/sandboxconfig/", function (req, _res, next) {
        req.baseDir = app.gameBase;
        next();
    }, sandboxConfigRoute);
    ...
    app.use("/configuration/", function (req, res, _next) {
    ...
    app.use("/basepath/", function (_req, res, _next) {
        res.send(app.gameBase);
    });
    app.use("/engine", function (req, res, _next) {
        ...
        res.render("engine", {
            ...
            engineFilesPath: "js/v".concat(version, "/").concat((0, utils_1.resolveEngineFilesVariable)(version), ".js")
        });
    });
    app.use("^\/test$", function (_req, res, _next) {
        res.redirect("/test/");
    });
    app.use("/test/*.js$", function (req, _res, next) {
        var ssn = req.session;
        req.baseDir = app.scenario.benchmarks[ssn.cntr].target;
        next();
    });
    app.use("/start/", function (req, res, _next) {
    ...
    app.use("/next/", function (req, res, _next) {
        var ssn = req.session;
    ...
    app.use("/finish/", function (req, res, _next) {
        var ssn = req.session;
    ...
    var errorHandler;
    if (isDev) {
        errorHandler = function (err, _req, res, _next) {
